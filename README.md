...
- `Lottery` Address of TBCC contract.
...
```shell script
# Close lottery.
0 12 * * SUN cd ~/lottery-scheduler && yarn execute:close:mainnet

# Draw lottery.
3 12 * * SUN cd ~/lottery-scheduler && yarn execute:draw:mainnet

# Start lottery.
5 12 * * SUN cd ~/lottery-scheduler && yarn execute:start:mainnet
```
